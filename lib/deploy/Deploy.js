...
const { configKeys } = require('./promptModules/config.js')
...
module.exports = class Deploy {
  ...
  getDeployConfig(env) {
    if (!fs.existsSync(this.configFile)) {
      errorLog('部署文件 deploy.config.json 不存在， 您可以使用 leaf deploy init 自动生成配置文件');
      return;
    }
    // 读取配置文件内容
    const config = fs.readJSONSync(this.configFile);
    const deployConfig = config[env];
    if (!this.checkEnv(deployConfig, env)) {
      return;
    }
    if (!deployConfig) {
  ...
  // 校验配置文件是否正确
  async checkEnv(deployConfig, env) {
    for (let i = 0; i < configKeys.length; i++) {
      if (!deployConfig[configKeys[i]]) {
        errorLog(`部署环境 ${env} 配置文件属性：${key} 为空`);
        console.log( );
        return false;
      }
    }
  ...
  async handleCredential(deployConfig, reCredential = false) {
    ...
    const file = path.join(home, '.leaf-credential.json');
    const key = `${host}-${port}`;
    let authInfo = null;
      ...
      const answers = await inquirer.prompt([
        {
          name: 'backup',
          type: 'confirm',
          message: `需要备份(${deployConfig.delpoyDir})吗?`,
          default: '',
        },
        {
          name: 'backupName',
